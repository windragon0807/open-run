...
export function AchievementButton({
  ...
  const buttonType = type || (status === '완료' ? 'completed' : status === '진행중' ? 'progress' : 'reward')
  ...
  const getButtonClass = (): string => {
    const baseStyle =
      'w-[60px] h-[30px] rounded-[8px] font-bold text-[12px] leading-[16px] tracking-[-0.02em] text-center'
  ...
  return (
    <button className={getButtonClass()} onClick={onClick} disabled={buttonType === 'completed'} type='button'>
      {getButtonText()}
    </button>
  )
}
